...
namespace MarioKartCombinationTesting
{
    class Program
    {
            ...
            { 
                ...
                if (characterNameCheck)
                {
                    ...
                    while (maxStatCheck)
                    {
                        ...
                        {
                            maxStatCheck = false;
                            statTypesUpper.Remove(maxStatMatch);
                        }
                        else
                            Console.WriteLine("This is not an eligible stat. Please choose one of: \nSPEED\nACCELERATION\nTRACTION\nHANDLING\nWEIGHT\nTURBO");
                    ...
                    while (minStatCheck)
                    {
                        ...
                        if (statTypesUpper.Contains(minStatMatch) && minStatMatch != maxStatMatch)
                            minStatCheck = false;
                        else if (minStatMatch == maxStatMatch)
                            Console.WriteLine("You cannot minimize the same stat that you want to maximize.");
                        else
                        {
                            Console.WriteLine("This is not an eligible stat. Please choose one of:");
                            foreach (string i in statTypesUpper)
                            {
                                Console.WriteLine(i);
                            }
                        }
                    ...
                    while (reRunFlag)
                    {
                        Console.WriteLine("Would you like to run again? (Yes/No)");
                        string reRunAnswer = Console.ReadLine();
                        string reRunAnswerMatch = reRunAnswer.ToUpper();

                        if (reRunAnswerMatch == "YES")
                        {
                            reRunFlag = false;
                        }
                        else if (reRunAnswerMatch == "NO")
                        {
                            reRunFlag = false;
                            runFlag = false;
                        }
      ...
      /*  static object[] NameCheck(string characterName)
        {
            string[] featherWeight = { "Baby Luigi", "Baby Mario", "Baby Peach", "Baby Daisy", "Baby Rosalina", "Lemmy", "Dry Bones" };
            string[] lightWeight = { "Toad", "Koopa", "Shy Guy", "Toadette", "Larry", "Wendy", "Bowser Jr", "Lakitu" };
            string[] medium1Weight = { "Peach", "Daisy", "Yoshi" };
